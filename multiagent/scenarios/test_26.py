...
class Scenario(BaseScenario):
    ...
    '''def create_wall(self, world, obstacle, index, x1, y1, x2, y2):
        # create a wall of obstacles
        if x2 - x1 == 0:
            y = y1
            if self.obstacle_count < index:
                obstacle.state.p_pos = np.array([x1, y])
            ...
            x = x1
            if self.obstacle_count < index:
                obstacle.state.p_pos = np.array([x, y1])
                obstacle.state.p_vel = np.zeros(world.dim_p)
                x += 0.1
                self.obstacle_count += 1'''
    ...
    def reset_world(self, world):
        ...
        for i, obstacle in enumerate(world.obstacles):
            if i == 0:
                obstacle.size = 0.1
            positions = [[0.00, 0.00], [-0.50, -0.50], [-0.50, 0.50], [0.50, -0.50], [0.50, 0.50]]
